ACC.cart = {
	...
	bindCartRemoveProduct: function ()
	{
		...
		{	
			...
			var productCode = $form.find('input[name=productCode]').val();
			var entryNum = $form.find('input[name=entryNumber]').val();		
			...
			if(initialCartQuantity != newCartQuantity)
			{
				...
				$.ajax({
					...
					{
						ACC.cart.refreshCartData(data, entryNum, newCartQuantity, itemIndex);
	...
	populateAndShowGrid: function(element, event, readOnly)
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                ACC.cart.coreTableActions(itemIndex, mapCodeQuantity);
        ...
        grid.on('focusout', skuQuantityClass, function(event) {
            ...
            if (quantityBefore != quantityAfter)
            {
            	...
            	$.ajax({
					...
					success: function(data, textStatus, xhr) 
					{
						ACC.cart.refreshCartData(data, -1, null, itemIndex);
